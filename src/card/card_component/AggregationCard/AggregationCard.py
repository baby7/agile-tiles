...
class AggregationCard(MainCard):
    ...
    def __init__(self, main_object=None, parent=None, theme=None, card=None, cache=None, data=None,
        ...
        self.util_app = None
        # 创建新的网络请求获取图标
        self.icon_network_manager = QtNetwork.QNetworkAccessManager(self)
        self.icon_network_manager.setCache(self.main_object.network_disk_cache)
        # 创建网络访问管理器
    ...
    def on_tab_changed(self, index):
        ...
        # 设置字体
        style_util.set_font_and_right_click_style(self.main_object, self.aggregation_tab_widget)
    ...
    def push_button_text_click(self, aggregation_module):
        try:
            ...
            print(e)
            self.main_object.info_logger.card_error(self.name, f"获取{str(aggregation_module)}失败,请稍后重试")
    ...
    def load_url_icon(self, url, image_label):
        ...
        reply.finished.connect(lambda : self.handle_icon_response(reply, image_label, url))
    ...
    def handle_icon_response(self, reply, image_label, original_url):
        try:
            if reply.error() == QtNetwork.QNetworkReply.NoError:
                # 获取网页内容
                html_content = str(reply.readAll().data(), 'utf-8')
                ...
                print("信息请求失败: " + reply.errorString())
            ...
            self.main_object.info_logger.card_error(self.name, f"解析HTML失败:{traceback.format_exc()}")
    ...
    def fetch_icon(self, icon_url, image_label):
        ...
        request.setAttribute(QtNetwork.QNetworkRequest.RedirectPolicyAttribute,
                             QtNetwork.QNetworkRequest.NoLessSafeRedirectPolicy)
        reply = self.icon_network_manager.get(request)
        reply.finished.connect(lambda : self.handle_icon_data(reply, image_label))
        self.reply_list.append(reply)
    ...
    def handle_icon_data(self, reply, image_label):
        try:
            ...
            self.main_object.info_logger.card_error(self.name, f"加载图标失败:{traceback.format_exc()}")
            self.set_default_icon(image_label)
