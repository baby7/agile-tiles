def card_has_change(old_data, new_data):
    ...
    """
    # 判断宽度是否改变
    old_big_card_width = old_data["data"]["SettingCard"]["width"]
    new_big_card_width = new_data["data"]["SettingCard"]["width"]
    if old_big_card_width != new_big_card_width:
        return True
    # 判断高度是否改变
    old_big_card_height = old_data["data"]["SettingCard"]["height"]
    new_big_card_height = new_data["data"]["SettingCard"]["height"]
...
def get_card_list_by_data_change(old_data, new_data):
    ...
    old_normal_card_list = old_data["card"]
    new_normal_card_list = new_data["card"]
    normal_card_data_update_list = compare_card_lists(old_normal_card_list, new_normal_card_list)

    # 获取有修改的主要卡片列表
    old_big_card_list = old_data["bigCard"]
    new_big_card_list = new_data["bigCard"]
    big_card_data_update_list = compare_card_lists(old_big_card_list, new_big_card_list)
