...
class FeedbackPopup(AgileTilesAcrylicWindow):
    ...
    def __init__(self, parent=None, use_parent=None, title=None, screen=None, current_user=None):
        ...
        self.feedback_manager = QNetworkAccessManager(self)  # 用于反馈提交
        # 当前上传请求
        self.current_upload = None
        self.current_file_path = None
        try:
            self.setWindowTitle("意见反馈" if title is None else title)
    ...
    def init_ui(self):
        ...
        self.title_input.setPlaceholderText("请输入反馈标题（50字以内）")
        ...
        self.content_input.setPlaceholderText("请详细描述您的问题或建议（1000字以内）")
    ...
    def create_icon(self):
        ...
        painter.drawText(QRect(0, 0, 32, 32), Qt.AlignCenter, "F")
        painter.end()
        return pixmap
            ...
            if self.feedback_reply.error() == QNetworkReply.NoError:
                ...
                if data is None or data == b'':
                    ...
                    if response_data.get("data"):
                        message_box_util.box_acknowledgement(self.use_parent, "提交成功", "您的反馈已提交，感谢您的宝贵意见！")
                        self.reset_form()
                    else:
    ...
    def closeEvent(self, event):
        ...
        # 取消反馈提交
        if hasattr(self, 'feedback_reply') and self.feedback_reply and self.feedback_reply.isRunning():
            self.feedback_reply.abort()
        super().closeEvent(event)


def show_feedback_dialog(main_object, title, current_user):
